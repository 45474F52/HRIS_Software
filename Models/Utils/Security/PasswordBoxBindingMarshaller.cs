...
namespace HRIS_Software.Models.Utils.Security
{
    /// <summary>
    /// Инкапсулирует методы для обновления пароля в <see cref="PasswordBox.SecurePassword" />
    /// </summary>
    internal sealed class PasswordBindingMarshaller
    {
        ...
        private void PasswordBoxPasswordChanged(object sender, RoutedEventArgs e)
        {
            if (!_isMarshalling)
            {
                ...
                try
                {
                    PasswordBoxBehaviour.SetSecurePassword(_passwordBox, _passwordBox.SecurePassword.Copy());
                }
                finally
                {
                    _isMarshalling = false;
